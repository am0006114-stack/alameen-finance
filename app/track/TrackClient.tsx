...
export default function TrackClient() {
  ...
  useEffect(() => {
    ...
    async function loadApplication() {
      ...
      if (!phone || !tracking) {
        setIsLoading(false);
        setErrorMessage("الرجاء إدخال رقم الهاتف ورقم التتبع معًا.");
        return;
      }

      if (!/^07[789]\d{7}$/.test(phone)) {
        setIsLoading(false);
        setErrorMessage("رقم الهاتف غير صحيح. يجب أن يبدأ بـ 079 أو 078 أو 077 ويكون من 10 أرقام.");
        return;
      }
      ...
      try {
        const response = await fetch(
          `/api/public/order-status?phone=${encodeURIComponent(phone)}&tracking=${encodeURIComponent(tracking)}`,
        ...
        setApplication({
          id: String(payload.trackingId || tracking),
          created_at: payload.createdAt || null,
          tracking_id: payload.trackingId || tracking,
          full_name: payload.customerName || null,
          phone: payload.phone || phone,
          status: payload.status || null,
          payment_status: payload.paymentStatus || null,
          payment_confirmed_at: payload.paymentConfirmedAt || null,
          device_name: payload.deviceName || null,
          device_price: payload.devicePrice ?? null,
          installment_months: payload.installmentMonths ?? null,
          down_payment: payload.downPayment ?? null,
          monthly_payment: payload.monthlyPaymentRaw ?? null,
          total_with_interest: payload.totalWithInterest ?? null,
          payment_reference: null,
          paid_clicked_at: payload.paidClickedAt || null,
        });
      } catch {
        if (!cancelled) {
          setErrorMessage("حدث خطأ أثناء البحث عن الطلب. حاول مرة أخرى.");
        }
      } finally {
  ...
  return (
    ...
    >
      ...
      <div className="relative mx-auto max-w-6xl">
        ...
        {isLoading && (
          <section className="mb-6 rounded-[28px] border border-[#d8c89f] bg-[#fffaf1] p-5 text-sm font-black leading-8 text-[#76591b] shadow-sm">
