...
async function exactCount(status?: string) {
  let query = supabaseAdmin.from("whatsapp_v2_archive_cases").select("id", { count: "exact", head: true });
...
export default async function ArchiveLabPage() {
  ...
  const [
    ...
    supabaseAdmin.from("whatsapp_v2_archive_cases")
      .select("id,source_created_at,customer_message,actual_reply,candidate_reply,status,actual_score,candidate_score,score_delta,winner,critical_actual,critical_candidate,failure_tags,total_cost_usd")
      .neq("status", "queued")
    ...
    exactCount(),
    exactCount("succeeded"),
    exactCount("needs_review"),
    exactCount("queued"),
    exactCount("budget_blocked"),
    exactCount("skipped"),
  ]);

  const settings = new Map<string,string>();
  for (const r of settingsRows || []) settings.set(String(r.key), String(r.value || ""));
  const rows = (recentEvaluated || []) as CaseRow[];
  const enabled = settings.get("lab_enabled") === "true";
  const spend = { deepseek: 0, openai: 0 };
  for (const u of usage || []) {
    const p = String(u.provider) as "deepseek"|"openai";
    if (p in spend) spend[p] += Number(u.estimated_cost_usd || 0);
  }
  ...
  return <main dir="rtl" className="min-h-screen bg-[#06110e] px-4 py-8 text-white"><div className="mx-auto max-w-7xl">
    <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><p className="text-sm font-black text-amber-200">ALAMEEN V2 — Phase 2.2</p><h1 className="text-3xl font-black">Archive Evaluation Lab</h1><p className="mt-2 max-w-3xl text-sm font-bold leading-7 text-[#b8c2bc]">Replay للأرشيف فقط. لا يرسل أي رد للعميل ولا يغيّر الطلب أو الدفع أو الإلغاء أو الاسترداد. السحب الآن Risk-first ليبدأ بالإلغاء/الاسترداد/الدفع/الكفيل/الشكاوى والمتابعات الصعبة قبل التحيات. العدادات Global لكل الأرشيف.</p></div><div className="flex flex-col gap-3"><ArchiveLabActions enabled={enabled}/><div className="flex gap-2"><Link href="/admin/whatsapp-v2-shadow" className="text-xs font-black text-cyan-200">V2 Shadow</Link><Link href="/admin/whatsapp" className="text-xs font-black text-cyan-200">المحادثات</Link></div></div></div>

    <section className="mb-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-9">{[
      ["الحالة", enabled ? "ON" : "OFF"], ["إجمالي الأرشيف", totalCount], ["queued", queuedCount], ["ناجح", succeededCount], ["مراجعة", reviewCount], ["موقوف ميزانية", blockedCount], ["skipped", skippedCount], ["V1 avg*", `${avgActual}%`], ["V2 avg*", `${avgCandidate}%`]
    ...
    <section className="mb-6 rounded-2xl border border-amber-300/20 bg-amber-950/15 p-4 text-sm font-bold leading-7 text-[#d7ddd8]">24h: DeepSeek <b>${spend.deepseek.toFixed(4)}</b> — OpenAI <b>${spend.openai.toFixed(4)}</b> — الإجمالي <b>${(spend.deepseek+spend.openai).toFixed(4)}</b>. الحدود الحالية: DeepSeek ${settings.get("deepseek_hourly_budget_usd")||"0.50"}/ساعة و${settings.get("deepseek_daily_budget_usd")||"2.00"}/يوم؛ OpenAI ${settings.get("openai_hourly_budget_usd")||"0.25"}/ساعة و${settings.get("openai_daily_budget_usd")||"1.00"}/يوم. *المتوسطات من آخر 250 حالة مقيمة المعروضة، بينما العدادات Global لكل الأرشيف.</section>
