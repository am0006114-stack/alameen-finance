...
export function enforceCustomerGenderLanguage(value: string, gender: CustomerGender): string {
  ...
  if (gender === "female") {
    ...
    reply = reply
      ...
      .replace(/(^|\s)ابعث(?:ي)?(?=\s|[،,.؟!:]|$)/g, "$1يمكن إرسال")
      .replace(/(^|\s)ارسل(?:ي)?(?=\s|[،,.؟!:]|$)/g, "$1يمكن إرسال")
      .replace(/(^|\s)أرسل(?:ي)?(?=\s|[،,.؟!:]|$)/g, "$1يمكن إرسال")
      .replace(/(^|\s)ارفع(?:ي)?(?=\s|[،,.؟!:]|$)/g, "$1يتم رفع")
      .replace(/(^|\s)ادفع(?:ي)?(?=\s|[،,.؟!:]|$)/g, "$1يتم دفع");
  }
