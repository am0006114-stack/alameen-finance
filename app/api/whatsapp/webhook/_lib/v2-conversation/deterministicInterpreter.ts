...
function explicitHandoff(text: string) {
  return v2HasAny(text, [
    "بدي موظف", "بدي موضف", "احكي مع موظف", "بدي انسان", "بدي بشر",
    "موظف حقيقي", "شخص حقيقي", "talk to a human", "live agent", "real person",
  ]);
}

function explicitCall(text: string) {
  return v2HasAny(text, [
    "بدي مكالمه", "بدي مكالمة", "اتصلوا في", "رنوا علي", "بدي احكي تلفون",
    "ممكن مكالمه", "ممكن مكالمة",
  ]);
}
...
function addQuestionTopics(acts: V2DialogueAct[], text: string) {
  ...
  const location = v2HasAny(t, [
    ...
    "عنوانكم", "موقع المكتب", "وين مكانكم",
  ]);
