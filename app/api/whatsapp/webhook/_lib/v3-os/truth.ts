...
export function resolveTruth(input: TruthResolverInput): TruthBundle {
  ...
  if (input.currentMessageTrackingId && input.conversationBoundApplication?.trackingId === input.currentMessageTrackingId) {
    return { confidence: "authoritative", source: "current_message_tracking", contactAccess: "full", application: input.conversationBoundApplication, ambiguousApplications: [], policy, fetchedAt };
  }
  if (input.state.activeApplicationId && input.conversationBoundApplication?.id === input.state.activeApplicationId) {
    return { confidence: "high", source: "conversation_binding", contactAccess: "full", application: input.conversationBoundApplication, ambiguousApplications: [], policy, fetchedAt };
  }
  if (input.uniquePhoneApplication && !(input.ambiguousApplications?.length)) {
    return { confidence: "high", source: "unique_phone_match", contactAccess: "full", application: input.uniquePhoneApplication, ambiguousApplications: [], policy, fetchedAt };
  }
  return { confidence: "none", source: "none", contactAccess: "none", application: null, ambiguousApplications: input.ambiguousApplications || [], policy, fetchedAt };
}
