...
function paymentAuthorityReply(input: { turn: InterpretedTurn; truth: TruthBundle }) {
  ...
  if (commercial === "already_paid" || stage === "payment_confirmed_under_review") {
    return `الدفع مؤكد إداريًا على طلبك، فما في داعي تدفع 5 دنانير مرة ثانية. والقسط الأول بيستحق بعد شهر من تاريخ توقيع العقد، وتاريخ توقيع العقد هو نفسه تاريخ استلام الجهاز.`;
  }
  if (commercial === "payment_pending_admin" || stage === "payment_proof_pending_admin") {
    return `وصل الدفع موجود على الملف وبانتظار المراجعة، فما في داعي تدفع مرة ثانية. والقسط الأول منفصل وبيستحق بعد شهر من تاريخ توقيع العقد، وتاريخ التوقيع هو نفسه تاريخ استلام الجهاز.`;
  }
  if (stage === "preliminary_approved_waiting_decision") {
    return `الـ5 دنانير مش دفعة أولى؛ هي رسوم فتح الملف، وبتصير فقط إذا اخترت الاستمرار. القسط الأول شيء منفصل وبيستحق بعد شهر من تاريخ توقيع العقد، وتاريخ توقيع العقد هو نفسه تاريخ استلام الجهاز.`;
  }
  if (commercial === "payment_ready" || stage === "continuation_confirmed_fee_due") {
    if (deferral) {
      return `إذا بدك نفتح الملف للدراسة النهائية، رسوم فتح الملف 5 دنانير لازم تنعمل بهالمرحلة قبل بدء الدراسة النهائية؛ ما بتنضاف على القسط الأول ولا بتتأجل معه. إذا مش جاهز هسا، ما في مشكلة ولا ضغط: لما تكون جاهز للخطوة بنكمل من نفس الطلب. القسط الأول بيستحق بعد شهر من تاريخ توقيع العقد، وتاريخ التوقيع هو نفسه تاريخ استلام الجهاز.`;
    }
    if (confusion) {
      return `صح، ما في دفعة أولى على الجهاز. الـ5 دنانير مش دفعة أولى؛ هي رسوم فتح الملف لبدء الدراسة النهائية. القسط الأول نفسه بيستحق بعد شهر من تاريخ توقيع العقد، وتاريخ توقيع العقد هو نفسه تاريخ استلام الجهاز.`;
    }
    if (wantsCurrentPayment) {
      ...
      return `نعم، إذا بدك تكمل فتح الملف هسا، المطلوب 5 دنانير رسوم فتح الملف. ${p.paymentMethodRule}${receipt ? `\nبعد التحويل ارفع الوصل من الرابط الرسمي المرتبط بطلبك:\n${receipt}` : ""}\nالقسط الأول مش مطلوب هسا؛ بيستحق بعد شهر من تاريخ توقيع العقد، وتاريخ توقيع العقد هو نفسه تاريخ استلام الجهاز.`;
    }
  }
  return `رسوم فتح الملف 5 دنانير مرتبطة بمرحلة ما بعد الموافقة المبدئية واختيار الاستمرار، وهي منفصلة عن القسط الأول. القسط الأول بيستحق بعد شهر من تاريخ توقيع العقد، وتاريخ التوقيع هو نفسه تاريخ استلام الجهاز.`;
}
