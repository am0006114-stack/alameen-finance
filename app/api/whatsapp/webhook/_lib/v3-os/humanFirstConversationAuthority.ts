...
export function currentFeePaymentQuestionText(value: string | null | undefined) {
  const q = n(value);
  if (!q) return false;
  return /(?:احول|أحول|ادفع|أدفع|بدفع|بحول).{0,28}(?:المصاري|الخمسه|الخمس|5|٥|رسوم\s+فتح\s+الملف).{0,25}(?:هسا|هلأ|الان|الآن|اليوم)?|(?:بدي|حاب|اريد|أريد).{0,20}(?:ادفع|أدفع|احول|أحول).{0,24}(?:الخمس|الخمسه|5|٥)|(?:الخمس|الخمسه|5|٥).{0,25}(?:هسا|هلأ|الان|الآن|ادفع|أدفع|احول|أحول)/.test(q);
}
