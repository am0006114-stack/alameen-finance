...
export function financingStructureQuestionText(value: string | null | undefined) {
  const q = normalized(value);
  return /(?:التقسيط|الاقساط|الأقساط|المعامله|المعاملة).{0,30}(?:عن\s+طريق|من\s+خلال).{0,18}(?:بنك|البنك)|(?:عن\s+طريق|من\s+خلال)\s+(?:بنك|البنك).{0,30}(?:التقسيط|الشروط|المعامله|المعاملة)|(?:بنك|البنك).{0,22}(?:وشو|وما|شو).{0,18}(?:الشروط|المتطلبات)|(?:نظامكم|التقسيط|المعامله|المعاملة).{0,30}(?:مرابحه|مرابحة|ربوي|ربا)|(?:مرابحه|مرابحة|ربوي|ربا).{0,30}(?:ولا|او|أو|نظام|التقسيط|المعامله|المعاملة)/.test(q);
}
