...
function instructionFor(topic: string, truth: TruthBundle, turn: InterpretedTurn) {
  ...
  const map: Record<string,string> = {
    ...
    review_timing: (() => { const q=String(turn.rawText||""); const beyond=/(?:بعد\s*(?:المده|المدة)|كم\s*يوم\s*(?:زياده|زيادة)|قديش\s*(?:زياده|زيادة))/i.test(q); return beyond ? `العميل لا يسأل عن المعدل الطبيعي نفسه بل عن الزيادة بعده. جاوبه مباشرة: لا يوجد رقم إضافي ثابت وموثق يمكن نسبه لكل الملفات، وأي رقم إضافي سيكون تخمينًا. اذكر أن الضغط الحالي شديد وأن بعض الملفات تتجاوز ${p.normalReviewWindow}، بدون إعادة نفس القالب أو وعد بتاريخ.` : `اذكر أن ${p.normalReviewWindow}. وضّح أيضًا أن ${p.severePressureRule} لا تعطِ يومًا أو ساعة محددة إذا لم يوجد ETA موثق. غيّر الصياغة حسب سياق العميل ولا تكرر قالبًا ثابتًا.`; })(),
...
export function buildReplyPlan(input: { turn: InterpretedTurn; state: ConversationState; truth: TruthBundle }): ReplyPlan {
  ...
  const objective = journeyStage === "refund_requested"
    ? "الطلب داخل مسار الاسترداد الآن. افهم سؤال العميل الحالي داخل هذا المسار فقط؛ ممنوع الرجوع إلى الاستمرار أو رسوم فتح الملف أو طلب دفع/وصل جديد. إذا المشكلة بالرابط أو بيانات الاسترداد، حلها مباشرة وبشكل بشري."
    : journeyStage === "refund_completed"
      ? "الاسترداد مكتمل حسب الحقيقة الحالية. لا تعيد فتح مسار دفع أو استمرار قديم، وجاوب فقط ما يسأل عنه العميل الآن."
      : journeyStage === "cancelled"
        ? "الطلب ملغي حاليًا. لا تعرض الاستمرار أو رسوم فتح الملف على نفس الحالة. إذا العميل يريد طلبًا جديدًا أو استردادًا، تعامل مع هذا الطلب الجديد/المسار وفق الحقيقة الحالية."
        : journeyStage === "payment_confirmed_under_review"
          ? "الدفع مؤكد والملف في الدراسة النهائية. لا تطلب 5 دنانير ولا وصلًا ولا «أود الاستمرار» مرة ثانية؛ جاوب السؤال الحالي من مرحلة الدراسة فقط."
          : journeyStage === "payment_proof_pending_admin"
            ? "وصل الدفع موجود وبانتظار اعتماد الإدارة. لا تطلب إعادة الدفع أو إعادة رفع الوصل أو قرار استمرار جديد؛ جاوب من هذه المرحلة الحالية."
            : journeyStage === "continuation_confirmed_fee_due"
              ? "اختيار الاستمرار مسجل. لا تطلب من العميل إعادة قرار الاستمرار. إذا سؤاله عن الدفع الحالي أعطه الحقيقة المسموحة، وإذا سؤاله عما بعد الدفع جاوب الخطوة التالية بدون إعادة block الدفع كاملًا."
              : journeyStage === "preliminary_approved_waiting_decision" && !explicitContinuation(input.turn)
                ...
                : journeyStage === "preliminary_review"
                  ? "اعرض حقيقة الطلب وحالته المبدئية فقط. لا تفتح موضوع الدفع أو رسوم فتح الملف أو قرار الاستمرار قبل صدور الموافقة المبدئية."
                  : "حل كل أجزاء رسالة العميل داخل نفس المحادثة وفق مرحلة الطلب الحالية. اقرأ فقاعات واتساب المتتالية كفكرة واحدة، ولا تعيد معلومة سبق أن أنهيتها. تفاصيل 5 دنانير تُعرض فقط بعد الموافقة المبدئية وموافقة العميل الصريحة على الاستمرار. عمران يملك التغييرات الحساسة، وتأكيد الدفع يدوي من الإدارة، وممنوع إعادة طلب مستندات وصلت أو استخدام رابط تتبع كرابط رفع.";
  return {
    objective,
