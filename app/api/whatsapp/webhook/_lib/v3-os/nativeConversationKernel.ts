...
const INTERNAL_LEAKS = [
  ...
  /اكتب\s+سؤالك\s+أو\s+رقم\s+التتبع/i,/احكيلي\s+شو\s+بدك\s+تعرف/i,/الحقيقة\s+الموثقة/i,
...
- أسماء فدوة/تالا/عبدالله/عبدالرحمن/عمران شخصيات موظفين للأمين واستمرارية صوت. يجوز «معك عمران من الأمين» عندما الدور فعلاً عمران ولم يُعرّف سابقًا، لكن ممنوع «أنا إنسان/موظف بشري» أو الادعاء أن تحويلًا بشريًا حصل. إذا طلب موظفًا، عالج السبب الذي جعله يطلبه ولا تدخل في نقاش تقني.
- ${emotionalOverlay}
...
- إذا الإفصاح وصل والعميل أكد الاستمرار بوضوح، أعطِ بيانات الدفع الرسمية كاملة ورابط الوصل الرسمي إن كان متاحًا.
...
export function validateNativeConversationReply(input: {
  ...
  if (/(?:ابعث|ابعت|ارسل|أرسل).{0,55}(?:الهويه|الهوية|كشف\s+راتب|شهاده\s+راتب|شهادة\s+راتب|وصل|اثبات\s+الدفع|إثبات\s+الدفع).{0,55}(?:واتساب|هون|هنا)/.test(n)) reasons.push("sensitive_document_requested_on_whatsapp");
  ...
  if (/أنا\s+(?:انسان|إنسان|موظف\s+بشري|الموظف\s+(?:المسؤول|المسوول))|انا\s+(?:انسان|موظف\s+بشري|الموظف\s+(?:المسؤول|المسوول))|حولتك\s+(?:لموظف|لشخص)|تم\s+تحويلك\s+(?:لموظف|لشخص)/.test(n)) reasons.push("false_literal_human_handoff_claim");
