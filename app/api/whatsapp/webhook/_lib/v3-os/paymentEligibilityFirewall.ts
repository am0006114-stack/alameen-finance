...
export function customerTextIsNonFeePaymentContext(value: string | null | undefined) {
  ...
  const installmentAdjustment = /(?:ازود|أزود|زياده|زيادة|ادفع|أدفع|اسدد|أسدد).{0,34}(?:القسط|الاقساط|الأقساط|الدفعات)|(?:القسط|الاقساط|الأقساط|الدفعات).{0,35}(?:ازود|أزود|زياده|زيادة|اكثر|أكثر|مقدم|مرتين|دفعتين|قسطين|شهرين)|(?:المبلغ\s+كامل|المبلغ\s+الكامل|ثمن\s+الجهاز\s+كامل|الجهاز\s+كامل).{0,45}(?:ادفع|أدفع|اسدد|أسدد|مره\s+وحده|مرة\s+وحدة)|(?:ادفع|أدفع|اسدد|أسدد).{0,35}(?:المبلغ\s+كامل|الجهاز\s+كامل|كل\s+المبلغ|مره\s+وحده|مرة\s+وحدة)|(?:شهر\s+عن\s+شهرين|قسط\s+عن\s+قسطين|ادفع\s+قسطين|أدفع\s+قسطين)|(?:تسديد|سداد).{0,25}(?:مبكر|كامل|بالكامل)/.test(q);
