...
export function explicitExpediteRequestText(value: string | null | undefined) {
  const q = n(value);
  return /(?:بلغ|بلّغ|وصل|وصلوا|احكي|احكوا).{0,35}(?:الاداره|الإدارة).{0,40}(?:مستعجل|مستعجله|مستعجلة|استعجال|سرعه|سرعة)|(?:بدي|بدنا|لو\s+سمحت|لو\s+سمحتوا|رجاء|رجاءا|رجائا|ارجو|أرجو).{0,34}(?:استعجال|تسريع|تسرعوا|تستعجلوا|تستعجل|سرعوا|سرّعوا).{0,30}(?:الطلب|القرار|المراجعه|المراجعة|الموافقه|الموافقة)?|(?:استعجلوا|استعجلو|تستعجلوا|تستعجلو).{0,24}(?:بالطلب|بلطلب|بالطلب|الطلب)|(?:اعملوني|اعملولي).{0,25}(?:حاله|حالة)\s+استثنائيه|(?:حاله|حالة)\s+استثنائيه.{0,25}(?:لو\s+سمحت|بدي)/.test(q);
}
