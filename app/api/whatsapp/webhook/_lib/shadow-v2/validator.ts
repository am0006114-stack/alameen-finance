...
function topicAnswered(topic: ShadowTopic, reply: string) {
  const checks: Partial<Record<ShadowTopic, string[]>> = {
    ...
    human_agent: ["عمران", "متابعه الحالات", "متابعة الحالات", "النقطه", "النقطة"],
...
export function validateShadowReply(
  ...
  if (includesAny(reply, [
    ...
    "اكتب النقطة بكلمتين",
    "ممكن توضح النقطة",
    "رح اجاوب على نفس النقطة",
    "لازم تدخل بشري",
    "سيتم تحويلك",
    "متابعة بشرية",
  ])) riskFlags.push("cold_or_internal_template");
  ...
  if (!facts.paymentCurrentlyAllowed && includesAny(reply, [
    "ادفعي رسوم فتح الملف",
    "ادفع رسوم فتح الملف",
    "اول خطوة رسوم فتح الملف",
    "أول خطوة رسوم فتح الملف",
    "حولي 5",
    "حوّل 5",
    "تحويل رسوم فتح الملف",
  ])) riskFlags.push("payment_requested_when_not_allowed");

  if (facts.paymentAlreadyConfirmed && includesAny(reply, ["ادفع الرسوم", "ادفعي الرسوم", "حول الرسوم", "حولي الرسوم"])) {
    riskFlags.push("duplicate_payment_request");
  }

  if (!facts.requiredDocument && includesAny(reply, [
    "نحتاج منك الهوية",
    "نحتاج منك كشف الراتب",
    "مطلوب منك كفيل",
    "ارفع الهوية",
    "ارفع كشف الراتب",
  ])) riskFlags.push("unrequested_document_claim");

  if (facts.requiredDocument !== "identity" && includesAny(reply, ["الهوية مطلوبة", "مطلوب رفع الهوية"])) riskFlags.push("wrong_identity_requirement");
  if (facts.requiredDocument !== "salary_slip" && includesAny(reply, ["كشف الراتب مطلوب", "مطلوب كشف راتب"])) riskFlags.push("wrong_salary_requirement");
  if (facts.requiredDocument !== "guarantor" && includesAny(reply, ["الكفيل مطلوب", "مطلوب كفيل"])) riskFlags.push("wrong_guarantor_requirement");

  if (!facts.officeAddressCanBeShared && includesAny(reply, ["رانا سنتر", "شارع المدينة المنورة", "مقابل مستشفى العيون"])) {
    riskFlags.push("address_shared_before_allowed_state");
  }

  if (includesAny(reply, [
    "اكيد بتقدري تسددي كامل",
    "أكيد بتقدري تسددي كامل",
    "حقك دائما تسددي",
    "حقك دائمًا تسددي",
    "السداد الكامل متاح دائما",
  ])) riskFlags.push("unsupported_early_settlement_guarantee");

  if (!facts.isApproved && includesAny(reply, ["موافقه نهائيه", "موافقة نهائية", "تمت الموافقة النهائية"])) {
    riskFlags.push("false_final_approval");
  }

  if (!facts.refundCompleted && includesAny(reply, ["تم الاسترداد", "رجع المبلغ", "تمت الحوالة"])) {
    riskFlags.push("false_refund_completion");
  }

  if (!facts.refundActive && includesAny(reply, ["تم تسجيل الاسترداد", "طلب الاسترداد مسجل"])) {
    riskFlags.push("false_refund_registration");
  }

  if (includesAny(reply, ["تواصلت مع المورد", "اتصلت بالمورد", "تم تصعيد الطلب"])) {
    riskFlags.push("unexecuted_action_claim");
  }
