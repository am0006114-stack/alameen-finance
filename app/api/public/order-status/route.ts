...
function getSafeStatus(app: ApplicationRecord, baseUrl: string) {
  ...
  if (app.status === "preliminary_qualified") {
    return {
      ...
      customerMessage:
        "طلبك مؤهل مبدئياً للانتقال إلى مرحلة الدراسة النهائية. للاستمرار، يلزم استكمال رسوم فتح الملف وفق التفاصيل الرسمية المرتبطة بطلبك. الرسوم مستردة بالكامل في حال عدم الموافقة، والقسط الأول لا يُدفع الآن بل بعد الاستلام حسب الاتفاق.",
      paymentInstruction:
        "اطلب من العميل استكمال رسوم فتح الملف وفق التفاصيل الرسمية، ثم رفع وصل الدفع من المسار المعتمد. لا تطلب القسط الأول الآن.",
    };
  }
  ...
  if (app.status === "customer_confirmed_continue") {
    return {
      ...
      actionUrl: receiptUploadUrl,
      customerMessage:
        "تم تسجيل رغبتك بالاستمرار. لاستكمال فتح الملف وتحويل الطلب للدراسة النهائية، يرجى استكمال رسوم فتح الملف وفق التفاصيل الرسمية المرتبطة بطلبك. الرسوم مستردة بالكامل في حال عدم الموافقة، والقسط الأول لا يُدفع الآن بل بعد الاستلام حسب الاتفاق.",
      paymentInstruction:
        "يمكن إرسال معلومات رسوم فتح الملف الرسمية ورابط رفع الوصل. لا تطلب القسط الأول.",
    };
  }
  ...
  if (
    ...
    return {
      ...
      actionUrl: receiptUploadUrl,
      customerMessage:
        "طلبك بانتظار استكمال رسوم فتح الملف وفق التفاصيل الرسمية المرتبطة بطلبك. الرسوم مستردة بالكامل في حال عدم الموافقة. بعد الدفع يرجى رفع أو إرسال وصل الدفع حتى يتم تحويل الطلب للدراسة النهائية.",
      paymentInstruction:
        "يمكن تذكير العميل بتفاصيل رسوم فتح الملف الرسمية ورابط رفع الوصل. لا تطلب القسط الأول.",
    };
  }
...
export async function GET(request: Request) {
  ...
  return NextResponse.json({
    ...
    safetyRules: {
      ...
      fileOpeningFeeAmount: "يظهر ضمن المرحلة المناسبة من الطلب",
