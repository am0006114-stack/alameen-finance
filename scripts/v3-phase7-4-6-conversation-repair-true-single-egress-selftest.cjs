...
// --- Phase 7.4.6 corpus-derived conversation repair + true single egress ---
r=ar('طيب وين مصاري الاسترداد لهسا؟','نعم، طلبك ملغي بالفعل، وطلب الاسترداد مسجل وقيد المعالجة.',app('refund_requested',{paymentStatus:'refund_requested'}),['refund']);
ok(r.obligation==='refund_timing','refund-money whereabouts becomes refund timing obligation');
ok(/مصاري الاسترداد لسا ما ظهرت/.test(r.reply||'')&&!/^نعم، طلبك ملغي بالفعل/.test(r.reply||''),'refund timing answers where the money is instead of repeating status');

r=ar('وين مصاري الالغاء','طلبك واضح: إعادة فتح الطلب بانتظار تنفيذ الإدارة.',app('refund_requested',{paymentStatus:'refund_requested'}),['refund']);
ok(r.obligation==='refund_timing','"وين مصاري الإلغاء" is informational refund timing, not reopen/cancel mutation');
ok(!/إعادة فتح/.test(r.reply||'')&&/الاسترداد/.test(r.reply||''),'refund-money question cannot be hijacked by reopen state');

r=ar('؟','نعم، طلبك ملغي بالفعل، وطلب الاسترداد مسجل وقيد المعالجة.',app('refund_requested',{paymentStatus:'refund_requested'}),[],[],state({lastCustomerText:'إسترداد ال٥ اميت؟'}));
ok(r.obligation==='refund_timing','punctuation/repair follow-up inherits the immediately previous refund question');
ok(/ما عندي موعد تحويل ثابت/.test(r.reply||''),'repair follow-up changes explanation instead of echoing the same template');

r=ar('هل لزم ادفع ٥ دنانير علشان اكمل','اختيار الاستمرار مسجل بالفعل. ما في داعي تعيد أود الاستمرار.',app());
ok(r.obligation==='fee_question','direct 5 JOD requirement becomes fee obligation');
ok(/^نعم/.test(r.reply||'')&&/رسوم فتح الملف/.test(r.reply||'')&&!/ما في داعي تعيد/.test(r.reply||''),'5 JOD requirement gets direct yes + purpose answer');

r=ar('ليه فتح الملف','رغبتك بالاستمرار مسجلة بالفعل',app());
ok(r.obligation==='fee_question','fee purpose question is explicit current obligation');
ok(/خطوة فتح الملف واستكماله للدراسة النهائية/.test(r.reply||''),'fee purpose is explained instead of stale continuation');

r=ar('ال٥ بترجعوهم؟','طلبك قيد الدراسة النهائية',app('customer_confirmed_continue',{paymentStatus:'confirmed',paymentConfirmedAt:'x'}));
ok(r.obligation==='fee_question','fee refundability question is detected');
ok(/مستردة عبر المسار الرسمي/.test(r.reply||''),'fee refundability gets a direct grounded answer');

r=ar('تمام يعني بدون دفعه اولا','طلبك قيد المراجعة المبدئية',app('submitted',{paymentStatus:null}));
ok(r.obligation==='fee_question','no-down-payment confirmation is treated as a direct commercial question');
ok(/^نعم، ما في دفعة أولى/.test(r.reply||'')&&/بعد شهر/.test(r.reply||''),'no-down-payment question gets direct yes and first-installment truth');

r=ar('تم دفع 5 دنانير\nما الإجراء بعد ذلك','رغبتك بالاستمرار مسجلة بالفعل',app('customer_confirmed_continue',{paymentStatus:'confirmed',paymentConfirmedAt:'x'}),['payment_status']);
ok(r.obligation==='post_payment_next_step','paid + what-next burst becomes post-payment obligation');
ok(/الدفع مؤكد إداريًا/.test(r.reply||'')&&/الدراسة النهائية/.test(r.reply||''),'confirmed payment explains the actual next stage');

r=ar('تم دفع 5 دنانير\nما الإجراء بعد ذلك','رغبتك بالاستمرار مسجلة بالفعل',app('customer_confirmed_continue',{paymentStatus:'pending_payment'}),['payment_status']);
ok(r.obligation==='post_payment_next_step','unconfirmed paid claim still gets post-payment handling');
ok(/ما بعتبر الدفع مؤكد من الرسالة نفسها/.test(r.reply||'')&&/الدراسة النهائية/.test(r.reply||''),'customer paid claim does not fabricate confirmation and still explains the next stage');

r=ar('بسأل اذا موجود عندك ايباد برو m5 اقساط\nالموقع ما في ايبادات','شوف الأجهزة من صفحة المنتجات',null,['products']);
ok(r.obligation==='product_availability','product absent from official site becomes availability obligation');
ok(/مش ظاهر بصفحة المنتجات الرسمية/.test(r.reply||'')&&/ما بقدر أعتبره متوفر حاليًا/.test(r.reply||''),'missing product is answered directly instead of looping the same link');

r=ar('طيب شو الاوراق المطلوبه او على الهويه ويوجد توصيل','الهوية وإثبات الدخل من الأساسيات.',null,['requirements','delivery']);
ok(r.obligation==='pickup_delivery','mixed requirements + delivery burst captures delivery obligation');
ok(/الهوية وإثبات الدخل/.test(r.reply||'')&&/ما في توصيل/.test(r.reply||''),'mixed question covers requirements and delivery together');

r=ar('هل تظهر شركتكم على نظام كريف في حال الاستعلام مستقبلا','رغبتك بالاستمرار مسجلة بالفعل',app(),[]);
ok(r.obligation==='external_system_question','CRIF/external-system question becomes explicit obligation');
ok(/ما عندي معلومة موثقة/.test(r.reply||'')&&/كريف/.test(r.reply||''),'unknown external-system truth is answered safely instead of ignored');

r=ar('إلغاء بشكل صريح','رغبتك بالاستمرار مسجلة بالفعل',app(),['cancellation']);
ok(r.obligation==='mutation_truth','explicit cancel wording is owned by mutation truth contract even if classifier is weak');
ok(r.repaired&&/أكدلي مرة واحدة/.test(r.reply||'')&&/ألغي الطلب/.test(r.reply||''),'stale continuation on explicit cancel is repaired to two-step confirmation');

ok(mut.explicitMutationRequest('cancel_application','إلغاء بشكل صريح'),'mutation gate recognizes customer wording "إلغاء بشكل صريح"');
const cancelledInfo=mut.enforceMutationConfirmationGate({actions:[],turn:turn('وين مصاري الالغاء',['refund']),state:state(),truth:truth(app('refund_requested',{paymentStatus:'refund_requested'}))});
ok(!cancelledInfo.informationalReply&&!cancelledInfo.blockedQuestionAction,'already-cancelled mutation gate does not swallow refund-money information question');

ok(gate.includes('internal_policy_instruction_leaked_to_customer'),'final gate blocks internal policy-instruction leakage');
ok(gate.includes('assistant_identity_defense_leaked'),'final gate blocks "مش رد آلي/مش روبوت" identity defense');
ok(!gate.includes('return "رغبتك بالاستمرار مسجلة بالفعل، فما في داعي تعيد خطوة «أود الاستمرار». جاوبني بالنقطة اللي بدك تعرفها وبكمل معك من المرحلة الحالية."'),'legacy stale continuation default removed from final replacement author');

const egressAt=runtime.indexOf('const egressArbitration = arbitrateProductionReply');
const finalSafetyAt=runtime.indexOf('const finalSafetyPass =');
ok(egressAt>runtime.indexOf('reply = finalGate.replacementReply'),'true single egress runs after any legacy final-gate replacement');
ok(egressAt>0&&egressAt<finalSafetyAt,'true single egress runs before final safety decision');
ok(runtime.includes('true_single_egress_repair'),'true single egress repairs are observable in telemetry');
const egressSlice=runtime.slice(egressAt,finalSafetyAt);
ok(!egressSlice.includes('reply = finalGate.replacementReply'),'after true single egress the safety gate is veto-only, not another author');

ok(types.includes('v3.0.0-phase7.4.6-conversation-repair-true-single-egress'),'runtime version identifies Phase 7.4.6');
ok(writer.includes('TRUE SINGLE EGRESS + CONVERSATION REPAIR'),'writer contract encodes conversation-repair semantics');
ok(writer.includes('إذا أعاد العميل نفس السؤال')&&writer.includes('لا تكرر نفس القالب'),'writer treats repeated question as failed previous answer');
ok(arbSrc.includes('Only explicit imperative customer language')&&arbSrc.includes('informational phrases'),'arbiter no longer trusts planner mutation labels over current customer wording');
ok(!runtime.includes('LIVE_SCOPED_MUTATIONS.add'),'7.4.6 does not expand Real Actions');
const all746=[runtime,gate,writer,types,arbSrc,mutation].join('\n');
ok(!/\b(INSERT|UPDATE|DELETE|ALTER|CREATE TABLE)\b/i.test(all746),'7.4.6 introduces no SQL mutation');
