...
let state={lastAssistantText:mismatch,lastCustomerText:'الغِ الطلب الآن AM-1788534808875',lastVerifiedApplication:null};
...
ok(/نكمل هون/.test(reply)&&/ما بقدر أعرض تفاصيل الطلب/.test(reply),'continuation keeps conversation open while blocking disclosure');
ok(/تنفيذ إداري/.test(reply)&&/ما رح أقول إنه تغيّر/.test(reply),'phone change is described as admin execution, never falsely completed');
ok(!/(?:قيد المراجعة|الدفع مؤكد|الجهاز)/.test(reply),'continuation leaks no foreign application truth');
...
ok(/أسترالي أو دولي/.test(reply)&&/مش مشكلة/.test(reply),'international number reason is acknowledged specifically, not with generic empathy');
...
ok(src.arb.includes('إذا الرقم المسجل إلك بس ما عليه واتساب')&&src.arb.includes('وبنكمل هون بالحل المناسب'),'initial mismatch reply invites a legitimate explanation instead of creating a dead end');
